...
import dft_local as dl
from dft_local.boltzmann_conductivity import (
    K_B_HARTREE_PER_K,
...
def identity_overlap_kernel(q: int = 1) -> dl.GdKernelArrays:
    return kernel(
        [0],
        [0],
        [0],
...
def cosine_k1_kernel(scale: float = 1.0, q: int = 1) -> dl.GdKernelArrays:
    return kernel(
        [1, -1],
        [0, 0],
        [0, 0],
...
def cosine_k2_kernel(scale: float = 1.0, q: int = 1) -> dl.GdKernelArrays:
    return kernel(
        [0, 0],
        [1, -1],
        [0, 0],
...
def diagonal_cosine_k1_kernel(scales: list[float]) -> dl.GdKernelArrays:
...
def shifted_overlap_k1_kernel(a: float) -> dl.GdKernelArrays:
    return kernel(
        [0, 1, -1],
        [0, 0, 0],
        [0, 0, 0],
...
def mixed_even_odd_kernel() -> dl.GdKernelArrays:
    return kernel(
        [0, 1, -2, 1],
        [0, -1, 2, 3],
        [0, 0, 1, 1],
...
def make_calc(
    KH: dl.GdKernelArrays,
    KS: dl.GdKernelArrays,
    k1,
    k2,
    *,
    weights=None,
...
def test_fermi_window_at_mu_in_au() -> None:
    ...
    window = fermi_window(E, mu=0.0, temperature=temperature, units=dl.AU)
...
def test_fermi_window_at_mu_in_evag() -> None:
    ...
    window = fermi_window(E, mu=mu, temperature=temperature, units=dl.eVag)

    kBT = K_B_HARTREE_PER_K * dl.eVag.E * temperature
...
def test_gd_symbol_derivatives_for_generic_pair_have_two_entries() -> None:
    ...
    pair = dl.SymbolPair(KH=KH, KS=KS, k1=0.1, k2=0.2)
...
def test_gd_symbol_derivatives_for_fixed_pair_have_two_entries() -> None:
    KH = cosine_k1_kernel()
    pair = dl.SymbolPair(KH=KH, KS=identity_overlap_kernel(), k1=0.1, k2=0.2, degree=1, sigma=1)
...
def test_gd_symbol_derivatives_rejects_missing_sigma_for_fixed_pair() -> None:
    KH = cosine_k1_kernel()
    pair = dl.SymbolPair(KH=KH, KS=identity_overlap_kernel(), k1=0.1, k2=0.2, degree=1, sigma=None)
...
def test_constructor_rejects_non_square_irrep_map() -> None:
    KH = cosine_k1_kernel()
    KS = identity_overlap_kernel()
    problem = dl.SymbolPair(KH=KH, KS=KS, k1=0.0, k2=0.0).form()
...
def test_hbar_units_scale_velocity() -> None:
    ...
    base = make_calc(KH, KS, [np.pi / 2], [0.0], units=dl.AU).run()
...
def scaled_kernel_energy(K: dl.GdKernelArrays, energy_scale: float) -> dl.GdKernelArrays:
    """
...
def test_same_physical_velocity_au_vs_evag_converts_by_length_over_time() -> None:
    ...
    time_unit_seconds = dl.eVag.hbar / dl.eVag.E
    expected_factor = dl.eVag.L / time_unit_seconds
...
def test_same_physical_fermi_window_au_vs_evag_converts_as_inverse_energy() -> None:
    ...
    assert np.allclose(win_ev, win_au / dl.eVag.E, rtol=1e-12, atol=1e-12)
...
def test_wrong_irrep_to_physical_k_changes_velocity_detectably() -> None:
    ...
    KH = scaled_kernel_energy(cosine_k1_kernel(), dl.eVag.E)
    KS = identity_overlap_kernel()
