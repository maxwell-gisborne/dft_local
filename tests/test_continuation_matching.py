...
def make_path(H, k1, k2=None, *, strategy="energy_predict") -> dl.LocalPath:
    ...
    return dl.LocalPath.from_arrays(
        H,
...
def test_hungarian_order_from_costs_minimises_cost() -> None:
    ...
    order = dl.hungarian_order_from_costs(costs)
...
def test_predicted_energies_from_history_constant_linear_quadratic() -> None:
    ...
    assert np.allclose(
        dl.predicted_energies_from_history(energies, 1, order=0),
        [0.0, 10.0],
    )
    assert np.allclose(
        dl.predicted_energies_from_history(energies, 2, order=1),
        [2.0, 6.0],
    )
    assert np.allclose(
        dl.predicted_energies_from_history(energies, 3, order=2),
        [3.0, 4.0],
    )
...
def test_match_via_energies_assigns_to_prediction_not_sorted_order() -> None:
    ...
    order, costs = dl.match_via_energies(E_raw, energies, 3, prediction_order=2)
...
def test_match_via_energies_rejects_shape_mismatch() -> None:
    ...
    with pytest.raises(ValueError, match="shape mismatch"):
        dl.match_via_energies(E_raw, energies, 1)
...
def test_fix_gauge_makes_diagonal_overlap_real_positive() -> None:
    ...
    U_fixed = dl.fix_gauge_against_previous_arrays(U_prev, S, U_curr)
...
def test_align_degenerate_group_with_reorder_aligns_rotated_subspace() -> None:
    ...
    U_aligned, singular_values, _order_after = dl.align_degenerate_group_with_reorder(
        U_prev,
        S,
        U_curr,
    )

    score = dl.eigenvector_overlap_scores(U_prev, S, U_aligned)
...
def test_energy_degenerate_groups_uses_connected_components() -> None:
    ...
    groups = dl.energy_degenerate_groups(E, gap_tol=0.06)
...
def test_align_groups_and_fix_gauge_aligns_group_and_phase_fixes_isolated_band() -> None:
    ...
    U_fixed, events = dl.align_groups_and_fix_gauge(
        U_prev,
        S,
        U_curr,
        E,
    ...
    score = dl.eigenvector_overlap_scores(U_prev, S, U_fixed)
...
def test_match_via_overlap_reports_degenerate_group_events() -> None:
    ...
    E, U, matched_scores, events = dl.match_via_overlap(
        prev_U,
        S,
        E_raw,
        U_raw,
    ...
    assert events[0].bands == (0, 1)
    assert dl.eigenvector_overlap_scores(prev_U, S, U).diagonal().min() > 1.0 - 1e-12
...
def test_local_region_from_parallelogram_shapes_and_strategy(fake_S: IdentityOverlapKernel) -> None:
    region = dl.LocalRegion.from_parallelogram(
