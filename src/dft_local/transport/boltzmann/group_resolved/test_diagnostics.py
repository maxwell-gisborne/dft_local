...
def test_group_resolved_overview_renders() -> None:
    ...
    assert "Density of states and neutral Fermi estimate" in html
    ...
    assert "final BigDFT FOE Fermi level" in html
    assert "final BigDFT FOE Tr(K)" in html
    assert "symbol mu - final BigDFT FOE eF" in html
    ...
    assert "stored-vs-recomputed relative error" in html
    assert "calc.velocities" in html
    assert "stored-vs-recomputed absolute error" in html
